...
def main(args):
	...
	robust_bound = np.zeros(len(samples))
	...
	for n,idx in enumerate(samples):
		...
		# 		print(idx, value)
		print('adv[0]:', adv[0])
		robust_bound[idx] = dist
		avg_dist += dist
		robust_incorrect += dist < args["epsilon"]
		print("\n======== Point {} ({}/{}) finished, distortion:{} =========".format(idx, n+1, num_attacks, dist))
	print('\n\nattacked {}/{} points, average linf distortion: {}, robust test error: {}, standard test error: {}, total time:{}, avg time:{}'.format(counter, num_attacks, avg_dist/counter, robust_incorrect/num_attacks, standard_incorrect/num_attacks, time.time()-global_start, (time.time()-global_start)/num_attacks))
...
if __name__ == '__main__':
	...
	parser.add_argument('-p', '--order', type=int, default=-1, help='order of the perturbation')
	...
	parser.add_argument('-e', '--epsilon', type=float, default=0.1, help='bounded perturbation')
